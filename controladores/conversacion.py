...
# Ejemplo de uso
consulta = "mi coche no arranca"
problemas_servicios = cargar_problemas_servicios()
problema, servicio, similitud = encontrar_problema(problemas_servicios, consulta)

if similitud >= 0.2:
    logging.info(f"Problema encontrado: {problema}, Servicio recomendado: {servicio}")
else:
    logging.info("No se encontró un servicio adecuado.")
...
def handle_message(message):
    ...
    if conversation_state["estado"] == "inicio":
        ...
        respuesta_bot = f"ℹ️ {respuesta_openai}. ¿ ℹ️ Hay algo más que quieras saber 📝 o deseas proceder con la reserva del servicio📅 '{conversation_state['servicio_principal']}'?"
        registrar_interaccion(conversation_state["usuario_id"], message, respuesta_bot, es_exitosa)
        conversation_state["estado"] = "confirmar_servicio"
        return respuesta_bot
